  ...
  <div class="cms-content-list">
    ...
    <el-row>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span class="btn-cell-wrap" v-if="scope.row.isPublish !== 'Y'">
              <el-button
                type="text"
                icon="el-icon-s-promotion"
                size="small"
                @click="handlePublish(scope.row)"
                >{{ $t("CMS.Site.PublishAll") }}</el-button
              >
            </span>
            <span class="btn-cell-wrap" v-else>
              <el-button
                type="text"
                icon="el-icon-s-promotion"
                size="small"
...
export default {
    ...
    return {
      ...
      rules: {
        name: [
          {
            required: true,
            message: this.$t("CMS.Site.RuleTips.Name"),
            trigger: "blur",
          },
        ],
        path: [
          {
            required: true,
            pattern: "^[A-Za-z0-9]+$",
            message: this.$t("CMS.Site.RuleTips.Path"),
            trigger: "blur",
          },
        ],
      },
  ...
  methods: {
    ...
    handlePublish(row) {
      publishSite({ siteId: row.siteId, publishIndex: true }).then(
        (response) => {
    ...
    handleUnPublish(row) {
      publishUnSite({ siteId: row.siteId, publishIndex: true }).then(
        (response) => {
          if (response.code == 200) {
