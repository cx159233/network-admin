<template>
  <div class="workorder-list-page">
    <PageHeader
      title="所有单据"
      description="查看平台所有工单，支持按工单编号、标题、类型和状态筛选，可查看工单详情"
    />

    <CloudCard class="workorder-list-page__table-card">
      ...
      <div class="workorder-list-page__divider"></div>

      <div class="workorder-list-page__table-wrap">
        <a-table
        ...
        <a-tabs v-model:activeKey="drawer.activeTab">
          <a-tab-pane key="basic" tab="基本信息">
            <a-descriptions :column="2" bordered size="small" class="drawer-desc">
              ...
              <a-descriptions-item label="当前处理人">{{ drawer.record.handler || '--' }}</a-descriptions-item>
              <a-descriptions-item label="申请时间">
                <span class="cell-mono">{{ drawer.record.createTime || '--' }}</span>
              </a-descriptions-item>
              <a-descriptions-item label="更新时间">
                <span class="cell-mono">{{ drawer.record.updateTime || '--' }}</span>
              </a-descriptions-item>
              <a-descriptions-item label="工单状态" :span="2">{{ drawer.record.status || '--' }}</a-descriptions-item>
...
export default {
  ...
  data() {
    return {
      ...
      workorderList: [
        { id: 1, code: 'WO20260402001', title: '网络服务申请', type: '业务申请', currentStep: '提交申请', status: '待处理', createTime: '2026-04-02 10:00:00', applicant: '张三', handler: '李四' },
        { id: 2, code: 'WO20260402002', title: '设备采购申请', type: '业务申请', currentStep: '部门审批', status: '处理中', createTime: '2026-04-02 09:30:00', applicant: '李四', handler: '王五' },
        { id: 3, code: 'WO20260401001', title: '系统权限申请', type: '业务申请', currentStep: '权限配置', status: '已完成', createTime: '2026-04-01 14:20:00', applicant: '王五', handler: '赵六' },
        { id: 4, code: 'WO20260331001', title: '技术支持请求', type: '技术支持', currentStep: '问题解决', status: '已关闭', createTime: '2026-03-31 11:00:00', applicant: '赵六', handler: '孙七' },
        { id: 5, code: 'WO20260330001', title: '问题反馈', type: '问题反馈', currentStep: '反馈处理', status: '处理中', createTime: '2026-03-30 16:00:00', applicant: '孙七', handler: '周八' }
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    resetQuery() {
      this.filter = { code: '', title: '', type: undefined, status: undefined }
      this.applied = { ...this.filter }
      this.pagination.current = 1
    },
    onTableChange(pag) {
    ...
    loadWorkorderList() {
      ...
      setTimeout(() => {
        this.workorderList = [
          {
            id: 1, code: 'WO20260402001', title: '网络服务申请', type: '业务申请', currentStep: '提交申请',
            status: '待处理', createTime: '2026-04-02 10:00:00', applicant: '张三', handler: '李四',
            updateTime: '2026-04-02 10:00:00', department: '技术部', contact: '13800138000', priority: '普通',
            description: '因业务需求，需要申请网络服务，用于公司内部系统访问。请相关部门尽快处理。',
            processSteps: [
              { title: '提交申请', time: '2026-04-02 10:00:00', handler: '张三', status: 'completed' },
              { title: '部门审批', time: '', handler: '李四', status: 'processing' },
          ...
          {
            id: 2, code: 'WO20260402002', title: '设备采购申请', type: '业务申请', currentStep: '部门审批',
            status: '处理中', createTime: '2026-04-02 09:30:00', applicant: '李四', handler: '王五',
            updateTime: '2026-04-02 11:00:00', department: '采购部', contact: '13800138001', priority: '高',
            description: '因部门业务扩展，需要采购一批办公设备，包括笔记本电脑、显示器等。',
            processSteps: [
              { title: '提交申请', time: '2026-04-02 09:30:00', handler: '李四', status: 'completed' },
              { title: '部门审批', time: '2026-04-02 10:00:00', handler: '王五', status: 'processing' },
              { title: '采购执行', time: '', handler: '', status: 'pending' },
              { title: '服务交付', time: '', handler: '', status: 'pending' }
            ],
            processRecords: [
              { time: '2026-04-02 09:30:00', content: '李四提交了设备采购申请工单', handler: '李四' },
              { time: '2026-04-02 10:00:00', content: '王五开始部门审批', handler: '王五' }
            ]
          },
          {
            id: 3, code: 'WO20260401001', title: '系统权限申请', type: '业务申请', currentStep: '权限配置',
            status: '已完成', createTime: '2026-04-01 14:20:00', applicant: '王五', handler: '赵六',
            updateTime: '2026-04-01 16:30:00', department: '运营部', contact: '13800138002', priority: '普通',
            description: '因工作需要，申请开通系统管理员权限，用于日常系统维护工作。',
            processSteps: [
              { title: '提交申请', time: '2026-04-01 14:20:00', handler: '王五', status: 'completed' },
              { title: '部门审批', time: '2026-04-01 15:00:00', handler: '赵六', status: 'completed' },
              { title: '权限配置', time: '2026-04-01 16:00:00', handler: '赵六', status: 'completed' },
              { title: '服务交付', time: '2026-04-01 16:30:00', handler: '赵六', status: 'completed' }
            ],
            processRecords: [
              { time: '2026-04-01 14:20:00', content: '王五提交了系统权限申请工单', handler: '王五' },
              { time: '2026-04-01 15:00:00', content: '赵六审批通过了工单', handler: '赵六' },
              { time: '2026-04-01 16:00:00', content: '赵六完成了权限配置', handler: '赵六' },
              { time: '2026-04-01 16:30:00', content: '工单已完成', handler: '赵六' }
            ]
          },
          {
            id: 4, code: 'WO20260331001', title: '技术支持请求', type: '技术支持', currentStep: '问题解决',
            status: '已关闭', createTime: '2026-03-31 11:00:00', applicant: '赵六', handler: '孙七',
            updateTime: '2026-03-31 15:00:00', department: '财务部', contact: '13800138003', priority: '紧急',
            description: '财务系统无法正常登录，请求技术支持协助排查。',
            processSteps: [
              { title: '提交申请', time: '2026-03-31 11:00:00', handler: '赵六', status: 'completed' },
              { title: '问题排查', time: '2026-03-31 13:00:00', handler: '孙七', status: 'completed' },
              { title: '问题解决', time: '2026-03-31 15:00:00', handler: '孙七', status: 'completed' }
            ],
            processRecords: [
              { time: '2026-03-31 11:00:00', content: '赵六提交了技术支持请求工单', handler: '赵六' },
              { time: '2026-03-31 13:00:00', content: '孙七开始排查问题', handler: '孙七' },
              { time: '2026-03-31 15:00:00', content: '孙七解决问题，工单关闭', handler: '孙七' }
            ]
          },
          {
            id: 5, code: 'WO20260330001', title: '问题反馈', type: '问题反馈', currentStep: '反馈处理',
            status: '处理中', createTime: '2026-03-30 16:00:00', applicant: '孙七', handler: '周八',
            updateTime: '2026-03-30 17:30:00', department: '市场部', contact: '13800138004', priority: '低',
            description: '系统页面加载缓慢，影响日常使用，请反馈处理。',
            processSteps: [
              { title: '提交申请', time: '2026-03-30 16:00:00', handler: '孙七', status: 'completed' },
              { title: '反馈处理', time: '2026-03-30 17:00:00', handler: '周八', status: 'processing' },
              { title: '服务交付', time: '', handler: '', status: 'pending' }
            ],
            processRecords: [
              { time: '2026-03-30 16:00:00', content: '孙七提交了问题反馈工单', handler: '孙七' },
              { time: '2026-03-30 17:00:00', content: '周八开始处理反馈', handler: '周八' }
            ]
          }
        ]
        this.loading = false
...
.workorder-list-page__divider {
  height: 1px;
  background: #F2F3F5;
  margin: 0 16px;
}

.workorder-list-page__table-wrap {
