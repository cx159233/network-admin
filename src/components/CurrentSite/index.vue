<template>
  <div v-if="showCurrentSite">
    <el-button
      ...
      plain
      >{{ currentSiteName }}</el-button
    >
    <el-dialog
      :title="$t('CMS.Site.CurrentSelectorTitle')"
      :visible.sync="open"
    ...
    >
      ...
      >
        ...
        <el-table-column
          :label="$t('CMS.Site.Name')"
          align="center"
          prop="name"
        >
          <template slot-scope="scope">
            <div slot="reference" class="name-wrapper">
              <el-tag v-if="scope.row.siteId == currentSite" size="medium">{{
                scope.row.name
              }}</el-tag>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleChangeCurrentSite">{{
          $t("Common.Confirm")
        }}</el-button>
...
<style>
#current-site .el-dialog__body {
  padding-top: 0;
}
#current-site .el-form-item {
  margin-bottom: 0;
}
</style>
<script>
